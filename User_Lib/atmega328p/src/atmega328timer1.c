...
{
	timer1_state = 0;
	tc1_instance()->tccr1a.var &= ~((1 << WGM11) | (1 << WGM10));
	tc1_instance()->tccr1b.var &= ~((1 << WGM13) | (1 << WGM12));
	switch(wavegenmode){
		case 0: // Normal
		break;
		case 1: // PWM, Phase Correct, 8-bit
			tc1_instance()->tccr1a.var |= (1 << WGM10);
		break;
		case 2:	// PWM, Phase Correct, 9-bit
			tc1_instance()->tccr1a.var |= (1 << WGM11);
		break;
		case 3:	// PWM, Phase Correct, 10-bit
			tc1_instance()->tccr1a.var |= (1 << WGM11) | (1 << WGM10);
		break;
		case 4:	// CTC
			tc1_instance()->tccr1b.var |= (1 << WGM12);
		break;
		...
		case 8:	// PWM, Phase and Frequency Correct
			tc1_instance()->tccr1b.var |= (1 << WGM13);
		break;
	...
	tc1_instance()->tccr1a.var &= ~((3 << COM1A0) | (3 << COM1B0));
	tc_imask_instance()->timsk1.var &= ~((1 << ICIE1)  | (1 << OCIE1B) | (1 << OCIE1A) | (1 << TOIE1));
	switch(interrupt){
		case 0:
		break;
		case 1:
			tc_imask_instance()->timsk1.var |= (1 << TOIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 2:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1A);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 3:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1B);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 4:
			tc_imask_instance()->timsk1.var |= (1 << ICIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 5:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1A) | (1 << OCIE1B);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 6:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1A) | (1 << TOIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 7:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1B) | (1 << TOIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 8:
			tc_imask_instance()->timsk1.var |= (1 << ICIE1) | (1 << TOIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 9:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 10:
			tc_imask_instance()->timsk1.var |= (1 << ICIE1) | (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
			cpu_instance()->sreg.var |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 11:
			tc_imask_instance()->timsk1.var |= (1 << OCIE1A) | (1 << OCIE1B);
	...
	setup_tc1.gcontrol_instance = tc_gcontrol_instance();
	setup_tc1.compare_instance = tc1_compare_instance();
	setup_tc1.imask_instance = tc_imask_instance();
	setup_tc1.iflag_instance = tc_iflag_instance();
	setup_tc1.instance = tc1_instance();	
	// V-table
...
void TIMER_COUNTER1_compoutmodeA(unsigned char compoutmode)
{
	tc1_instance()->tccr1a.var &= ~(3 << COM1A0);
	switch(compoutmode){ // see table 53, 54, 55 in data sheet for more information
		...
		case 1: // Reserved
			...
			portb_instance()->ddr.var |= (1 << 1);
			tc1_instance()->tccr1a.var |= (1 << COM1A0);
		break;
		case 2: // Clear OC1 on compare match when up-counting.
			...
			portb_instance()->ddr.var |= (1 << 1);
			tc1_instance()->tccr1a.var |= (1 << COM1A1);
		break;
		case 3: // Set OC1 on compare match when up-counting.
			...
			portb_instance()->ddr.var |= (1 << 1);
			tc1_instance()->tccr1a.var |= (1 << COM1A0) | (1 << COM1A1);
		break;
...
void TIMER_COUNTER1_compoutmodeB(unsigned char compoutmode)
{
	tc1_instance()->tccr1a.var &= ~(3 << COM1B0);
	switch(compoutmode){ // see table 53, 54, 55 in data sheet for more information
		case 0: // Normal port operation, OC1 disconnected.
		break;
		case 1: // Reserved
				// Toggle OC1 on compare match
			portb_instance()->ddr.var |= (1 << 2);
			tc1_instance()->tccr1a.var |= (1 << COM1B0);
		break;
		case 2: // Clear OC1 on compare match when up-counting.
				// Set OC1 on compare
				// match when down counting.
				// Clear OC1 on compare match
			portb_instance()->ddr.var |= (1 << 2);
			tc1_instance()->tccr1a.var |= (1 << COM1B1);
		break;
		case 3: // Set OC1 on compare match when up-counting.
				// Clear OC1 on compare
				// match when down counting.
				// Set OC1 on compare match
			portb_instance()->ddr.var |= (1 << 2);
			tc1_instance()->tccr1a.var |= (1 << COM1B0) | (1 << COM1B1);
		break;
		default: // Normal port operation, OC1 disconnected.
		break;
	}
...
void TIMER_COUNTER1_compareA(uint16_t compare)
{
	tc1_compare_instance()->ocr1a.var = writehlbyte(compare).var;
}
void TIMER_COUNTER1_compareB(uint16_t compare)
{
	tc1_compare_instance()->ocr1b.var = writehlbyte(compare).var;
}
void TIMER_COUNTER1_stop(void)
// stops timer by setting prescaler to zero
{
	tc1_instance()->tccr1b.var &= ~(7 << CS10); // No clock source. (Timer/Counter stopped)
	tc1_instance()->tcnt1.var = writehlbyte(0X0000).var;
	timer1_state = 0;
}
